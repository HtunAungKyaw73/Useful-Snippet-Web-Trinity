...
stories.forEach((story) => {
  ...
  section.innerHTML = `
      <div class="story-text" data-img-index="${story.imgIndex}" style="color: ${story.color}">
        <h1>${story.title}</h1>
  ...
  (entries) => {
    ...
    entries.forEach(async (entry) => {
      if (entry.isIntersecting) {
        // const index = Array.from(scrollContent.children).indexOf(entry.target);
        const index = parseInt(entry.target.children[0].dataset.imgIndex);
        if (index !== currentIndex) {
          currentIndex = index;
          const story = stories[currentIndex];
          bgBlack.style.backgroundColor = story.color;
